...
.config(['$urlRouterProvider', '$locationProvider', function($urlRouterProvider, $locationProvider) {
  ...
  .run(['superCache', function(superCache){
    superCache.put('auth',{
      /*
    parola : 'papero',
    autorizzato: true
...
.controller('listaimageCtrl', ['$scope', '_', 'httpFactory', '$mdDialog', function($scope, _, httpFactory, $mdDialog) {
  ...
  $scope.setState = function(imageToSet) {

    httpFactory.Save('image', imageToSet).success(function(data) {
      console.log(data);
    });
  };
...
.controller('galleriaCtrl', ['$scope', 'httpFactory', '_', function($scope, httpFactory, _) {
  $scope.multiplier= 5;
  httpFactory.GetList('image/active')
  .success(function(data) {
    console.log(data);

  var items=  _(_.filter(data, { 'attivo': true})).forEach(function(n) {
...
.controller('nuovopostCtrl', ['$scope', '_', 'httpFactory', '$state', '$stateParams', '$mdDialog', '$filter', function($scope, _, httpFactory, $state, $stateParams, $mdDialog, $filter) {
  ...
  httpFactory.GetId('post', $stateParams.id)
    .success(function(data) {
      var d = $filter('date')(data.date, "yyyy-mm-dd");
      data.date = new Date(data.date);
      $scope.post = data;
      console.log($scope.post.date);

      httpFactory.GetId('image', data._id)
        .success(function(data) {
          $scope.imgGalleria = data;
        });
  ...
  $scope.tags = function() {
    var tagList = [];
    if ($scope.post && $scope.post.tags) {
      tagList = $scope.post.tags;
    }
    var dialogConfig = {
  ...
  $scope.DeleteImageGallery = function(id, index) {
    httpFactory.Delete('image', id)
      .success(function(data) {
        $scope.imgGalleria.splice(index, 1);
      });
  };
...
.controller('listapostCtrl', ['$scope', '_', 'httpFactory', '$mdDialog', function($scope, _, httpFactory, $mdDialog) {
  ...
  var tipiDiPost = ['articolo', 'outfit', 'shooting', 'press', 'aboutme', 'travel'];
  $scope.tabs = [];

  var populateTabs = function(groupedData) {


    $scope.tabs = [];
    _.forEach(tipiDiPost, function(n) {
      $scope.tabs.push({
        label: n,
        data: groupedData[n]
      });
  ...
  httpFactory.GetList('post')
    .success(function(data) {
      ...
      populateTabs(_.groupBy(data, 'tipo'));
    });
    ...
    $mdDialog.show(dialogConfig)
      .then(function(postToAdd) {
        httpFactory.New('post', postToAdd)
          .success(function(data) {
            _.forEach($scope.tabs, function(n) {
              if (n.label === data.tipo){
                n.data.unshift(data);
              }
            });
          });
...
.controller('pressCtrl', ['$scope', 'superCache', '_', function($scope, superCache, _) {

  $scope.multiplier = 9;

  var data = superCache.get('press');
  var chankIterator = 0;
  $scope.pressList = [];
  $scope.moreAviable = data.length > chankIterator;
...
.controller('postSingleCtrl', ['$scope', 'httpFactory', '$stateParams', 'superCache', '_', function($scope, httpFactory, $stateParams, superCache, _) {
  ...
  var item = _.find(superCache.get('post'), {
    '_id': $stateParams.id
  });
...
.controller('travelCtrl', ['$scope', 'superCache', '_', function($scope, superCache, _) {
  ...
  var data = superCache.get('travel');
  var chankIterator = 0;
  $scope.articoliList = [];
  $scope.moreAviable = data.length > chankIterator;
...
app
  .factory('httpFactory', ['_', '$http', '$upload', 'superCache', function(_, $http, $upload, superCache) {


//    var urlBase = 'http://localhost/api';
  var urlBase = 'http://ec2-52-11-54-244.us-west-2.compute.amazonaws.com/api';

$http.defaults.headers.common.magicWord = superCache.get('auth').parola;
    var dataFactory = {};
...
app
  .directive('tagBotton', function() {
    ...
    return {
      ...
      template: '<md-button  style="text-transform: capitalize; " ui-sref="taglist({tagToSearch: rif})" class="md-raised md-primary botton-tag">{{rif}}</md-button>'
...
.controller('backendCtrl', ['$scope', 'superCache', 'httpFactory', '$http', function($scope, superCache, httpFactory, $http) {
  $scope.stato = 'in attesa';
  $scope.autorizzato = superCache.get('auth').autorizzato;
  $scope.autorizza = function(parola) {
    $http({
        method: 'GET',
        url: httpFactory.urlBase + '/cache/auth',
        headers: {
          magicWord: parola
        }
      })
      .success(function() {
        $scope.stato = 'si, sei autorizzato';
        $scope.autorizzato = true;
        $http.defaults.headers.common.magicWord = parola;
          superCache.put('auth',{
            parola : parola,
            autorizzato: true
          });
      ...
      .error(function() {
        $scope.stato = 'no, non sei autorizzato';
      });
  };
}]);
...
// Directive     <hiding-menu></hiding-menu>
app
  .directive('hidingMenu', ['$mdSidenav', function($mdSidenav) {

    function link(scope, element, attrs) {


      scope.closeNav = function() {
        $mdSidenav('hidingBar').close();
      };


      scope.hidingMenuItems = [{
          link: 'home',
          text: 'home'
        }, {
          link: 'articoli',
    ...
    return {
      restrict: 'E',
      templateUrl: 'base/directives/hidingMenu/tpl.html',
      link: link
    };
  }]);

// Directive     <hiding-nav></hiding-nav>
app
  .directive('hidingNav', ['$mdSidenav', function($mdSidenav) {

    function link(scope, element, attrs) {
      scope.toggleNav=function(){
        $mdSidenav('hidingBar').open();
      };
    }

    return {
      restrict: 'E',
      templateUrl: 'base/directives/hidingNav/tpl.html',
      link: link
    };
  }]);
...
app
  .directive('sidenav', ['$state', '_', '$rootScope', function($state, _, $rootScope) {
    ...
    function link(scope, element, attrs) {
      ...
      var desappearInThose=[ 'galleria', 'aboutme', 'postSingle'];
...
{link: 'www.facebook.com/pages/Jessica-mura/325063257659864', icon: 'facebook'},

{link: 'www.pinterest.com/jessicamura86/', icon: 'pinterest'},

{link: 'www.instagram.com/jessicamura/', icon: 'instagram'},

{link: 'www.twitter.com/86arumacissej', icon: 'twitter'},

{link: 'plus.google.com/113654009080986751320', icon: 'google-plus'}

// ,{link: 'plus.google.com/113654009080986751320', icon: 'youtube'}
...
app.config(['$stateProvider', function($stateProvider) {
  ...
  function toResolve(superCache, httpFactory, _) {
    ...
    .success(function(data) {
      ...
      superCache.put('articoli',_.chunk(_.filter(data.post, { 'tipo': 'articolo'}) , 3));
      superCache.put('outfit',_.chunk(_.filter(data.post, { 'tipo': 'outfit'}) , 3));
      superCache.put('shooting',_.chunk(_.filter(data.post, { 'tipo': 'shooting'}) , 3));
      superCache.put('press',_.chunk(_.filter(data.post, { 'tipo': 'press'}) , 6));
      superCache.put('travel',_.chunk(_.filter(data.post, { 'tipo': 'travel'}) , 3));
...
angular.module('comment')
  .directive('comment', ['httpFactory', '_', function(httpFactory, _) {
    ...
    function link(scope, element, attrs) {
      ...
      scope.Save = function() {
        ...
        scope.commento.rif=attrs.rif;
        httpFactory.New('comment', scope.commento)
          .success(function(data) {

            scope.commenti.unshift(data);

            scope.commento.nome = "";
            scope.commento.mail = "";
            scope.commento.testo = "";
          });
      };
    }
...
// Directive     <galleria rif="{{_id}}"></galleria>
angular.module('image')
  .directive('galleria', ['httpFactory', '_', function(httpFactory, _) {
    function link(scope, element, attrs) {
    ...
    scope.multiplier=9;
      scope.$watch('rif', function(newValue, oldValue) {
        ...
        if (newValue === '') {
          ...
          httpFactory.GetId('image', newValue)
          .success(function(data){
            ...
            console.log('immagini relative '+JSON.stringify(data));
    ...
    return {
      ...
      restrict: 'E',
      templateUrl: 'image/directives/gallery/tpl.html',
      link: link
    };
  }]);
...
// Directive     <ownlike></ownlike>
app
  .directive('ownlike', ['httpFactory', function(httpFactory) {

    function link($scope, element, attrs) {

      $scope.piuUno = function(tipo){
        httpFactory.GetList('post/addtocounter/'+$scope.articoli._id+'/'+tipo)
        .success(function(data) {

        $scope.articoli.contatori[tipo]+=1;
        });
    ...
    return {
      restrict: 'AE',
      templateUrl: 'post/directives/ownlike/tpl.html',
      link: link
    };
  }]);
...
.controller('pageSliderController', ['$scope', '$timeout', 'httpFactory', '$interval', function($scope, $timeout, httpFactory, $interval) {

  httpFactory.GetId('image', $scope.articoli._id)
    .success(function(data) {
...
// Directive     <slider></slider>
angular.module('slider')
  .directive('slider', ['superCache', '_', '$rootScope', '$state', 'sliderFaktory', '$stateParams', '$interval', function(superCache, _, $rootScope, $state, sliderFaktory, $stateParams, $interval) {
    function link(scope, element, attrs) {

      var promiseAA;
      scope.startAA = function() {
        scope.stopAA();
        promiseAA = $interval(function() {
          $('.topslider').slick('slickNext');
        }, 4000);
      };
      scope.stopAA = function() {
        $interval.cancel(promiseAA);
      };
