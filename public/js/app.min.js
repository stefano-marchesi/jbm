...
.controller('articoliCtrl', ['$scope', 'superCache', '_', function($scope, superCache, _) {
    ...
    $scope.articoliList = superCache.get('post');
...
.controller('outfitCtrl', ['$scope', 'superCache', '_', function($scope, superCache, _) {
  $scope.articoliList = _.filter(superCache.get('post'), { 'tipo': 'outfit'});
...
.controller('shootingCtrl', ['$scope', 'superCache', '_', function($scope, superCache, _) {
  $scope.articoliList = _.filter(superCache.get('post'), { 'tipo': 'shooting'});
...
app
  ...
  //  var urlBase = 'http://19ad9b24.ngrok.com/api';
...
app
  .directive('sidenav', ['$state', '_', '$rootScope', function($state, _, $rootScope) {
    ...
    function link(scope, element, attrs) {
      ...
      var desappearInThose=['outfit', 'galleria', 'shooting'];
...
{link: 'contact', icon: 'facebook-alt'},

{link: 'tags', icon: 'pinterest'},

{link: 'post', icon: 'instagrem'},

{link: 'post', icon: 'twitter-square'}
...
app.config(['$stateProvider', function($stateProvider) {
  ...
  function toResolve(superCache, httpFactory) {
  ...
  toResolve.$inject = ['superCache', 'httpFactory'];
